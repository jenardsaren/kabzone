<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-900">Create Session (Admin)</h2>
    </x-slot>

    <div class="py-8">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <div class="mb-4 rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
                Admin scheduling uses the same conflict and operating-hour rules as Front Desk.
            </div>
            ...
            <div class="rounded-lg bg-white p-6 shadow-sm" x-data="{ mode: '{{ old('schedule_mode', 'single') }}' }">
                <form method="POST" action="{{ route('admin.sessions.store') }}" class="space-y-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ route('admin.dashboard') }}" class="text-sm text-gray-600 hover:text-gray-900">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
