<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-xl font-semibold text-gray-900">Staff Accounts</h2>
            <a href="{{ route('admin.staff.create') }}" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">
                New Staff
            </a>
        </div>
    </x-slot>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($staff as $member)
                                <tr>
                                    ...
                                    <td class="px-3 py-2">{{ str($member->role->value)->headline() }}</td>
                                    <td class="px-3 py-2"><x-status-badge :status="$member->status" /></td>
                                    <td class="px-3 py-2">
                                        <a href="{{ route('admin.staff.edit', $member) }}" class="text-indigo-600 hover:text-indigo-500">Edit</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-3 py-6 text-center text-gray-500">No staff accounts found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
